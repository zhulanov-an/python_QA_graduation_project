...
class BasePage:
    ...
    def open(self):
        with allure.step(f"открытие страницы по адресу '{self._url}'"):
    ...
    def title(self):
        ...
        with allure.step(f'значение заголовка браузера - "{title}"'):
            return title
    ...
    def _get_element_text(self, selector, index=0):
        ...
        with allure.step(f'получено значение "{text}"'):
            return text
    ...
    @allure.step('получение элемента страницы')
